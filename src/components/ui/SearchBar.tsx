...
const SearchBar = ({ onSearch }: SearchBarProps) => {
  ...
  return (
    <motion.div 
      className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg p-1 transition-all duration-300 ${
        isFocused ? 'ring-2 ring-primary-300 dark:ring-primary-500' : ''
      }`}
      ...
      <form onSubmit={handleSubmit} className="flex flex-col md:flex-row">
        <div className="flex-1 flex items-center px-4 py-3 border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-700">
          ...
          <input
            ...
            className="w-full focus:outline-none bg-transparent dark:text-gray-100 dark:placeholder-gray-500"
            value={query}
        ...
        <div className="p-2 flex">
          {(query || location || date) && (
            <button 
              type="button" 
              className="mr-2 p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 rounded-md transition-colors"
              onClick={clearSearch}
          ...
          <motion.button
            type="submit"
            className="bg-primary-500 hover:bg-primary-600 dark:bg-primary-600 dark:hover:bg-primary-700 text-white px-6 py-2 rounded-lg transition-colors flex-shrink-0"
